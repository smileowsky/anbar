...
def loader(request):
    ...
    if 'x' in request.POST:
        ...
        #Suplplier add end
            ...
            #Suplplier edit without refresh
            if 'edit_id' in request.POST:
                edit_data = Supplier.objects.get(id=request.POST['edit_id'])
            #Suplplier edit done
                
            #Suplplier update without refresh
            if 'update' in request.POST:
                if 'update':
                    del_all = request.POST.getlist('images[]')
                    for foto in del_all:
                        Images.objects.get(aaaaaimage=foto).delete()

                    update = Supplier.objects.get(id=request.POST['id'])
                    if Supplier.objects.filter(supplier_email=request.POST['supp_email']).exclude(id=update.id).exclude(supplier_email=request.POST['supp_email']):
                        ...
                        if 'sup_photo' in request.FILES:
                            new_photo = request.FILES['sup_photo']
                            update.supplier_photo = new_photo
                            filesystem = FileSystemStorage()
                            file = filesystem.save(new_photo.name, new_photo)
                    ...
                    messages.info(
                        request, "Empty fields", extra_tags='error')
            #Supplier update done
            ...
            #Supplier delete selected without refresh
        ...
        #Position add end
            ...
            #Position edit without refresh
            if 'edit_id' in request.POST:
                edit_data = Positions.objects.get(id=request.POST['edit_id'])
            #Position edit done

            #Position update without refresh
            if 'update' in request.POST:
                if 'update':
                    update = Positions.objects.get(id=request.POST['id'])
                    update.positions = request.POST['position_name']
                    update.dep_id = Departments.objects.get(id=request.POST['department_id'])
                    update.save()
                    messages.info(request, "Update was successful.", extra_tags='success')
                else:
                    messages.info(request, "Empty field.", extra_tags='error')
            #Position update done
        ...
        #Staff add end
            ...
            #Staff edit without refresh
            if 'edit_id' in request.POST:
                edit_data = Staff.objects.get(id=request.POST['edit_id'])
            #Staff edit done
